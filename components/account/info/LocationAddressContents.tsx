...
import { setDistrict, setPlaces, setRegion, setWard } from '../accountSlice'




export function LocationAddressContents() {


  const { width, height } = useWindowDimensions();
  const dispatch = useAppDispatch();
  const { country, region, count, district, ward, places } = useAppSelector(state => state.account.userinfo)
  //location list
  ...
  return (
    <YStack gap={"$2"} width="100%">
      ...
      <Select
        value={places}
        onValueChange={(text) => dispatch(setPlaces(text))}
